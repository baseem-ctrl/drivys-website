    ...
    <section className="w-full bg-black py-12 lg:py-20 relative overflow-hidden">
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 lg:gap-6 mb-12">
        <div className="flex flex-col gap-3">
          <Button text="Testimonials" className="text-white text-2xl font-normal px-4 py-2 rounded-lg w-fit" />
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        <div
          className="grid grid-cols-3 gap-6 overflow-hidden relative"
          style={{
            height: "600px",
        ...
        >
          {columns.map((columnTestimonials, colIndex) => (
            <div
              ...
              style={{ animationDuration: `${40 + colIndex * 10}s` }}
            >
              {columnTestimonials.map((testimonial, index) => (
                <div
                  key={`${colIndex}-${index}`}
                  className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-5 flex flex-col gap-3 cursor-pointer min-h-[120px] transition-all duration-200 hover:bg-[#252525] hover:translate-y-[-2px] hover:shadow-[0_8px_25px_rgba(0,0,0,0.4)]"
                >
                  <div className="flex items-center gap-3">
                    <img src={testimonial.image} alt={testimonial.name} className="w-12 h-12 rounded-full border-2 border-[#333] object-cover" />
                    <div className="flex flex-col flex-1">
                      ...
                      <span className="text-[#666] text-xs">{testimonial.username}</span>
                    </div>
                  </div>
                  <p className="text-[#e1e5e9] text-sm">{testimonial.text}</p>
                </div>
              ))}
            </div>
          ))}
